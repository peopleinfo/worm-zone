...
export const MiniMap: React.FC<MiniMapProps> = ({ className = "" }) => {
  ...
  // Frame rate limiting for 20 FPS
  const lastRenderTimeRef = useRef<number>(0);
  const FRAME_INTERVAL = 1000 / 20; // 50ms per frame

  // Get game state from Zustand store
  const mySnake = useGameStore((state) => state.mySnake);
  const otherSnakes = useGameStore((state) => state.otherSnakes);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Only render if enough time has passed (20 FPS limiting)
